...
"""
##############################################################################
This is good code, but as written won't work with the only test file I have.
For now, I have copy-pasted the code and changed it slightly to recognize the
sheet that I have, allowing the Upload model to launch Upload_Handler.
We will need to revisit these methods at a later date, when we have clarified
with our collaborators/clients as to the format of input files. -Alex
##############################################################################
...
"""###########################################################################
###          TEMPORARY COPY OF FUNCTION guess_filetype()                   ###
###########################################################################"""
def guess_filetype(unknown_file):
    #File types accepted: Sample Metadata (csv), Replicate Metadata (csv),
    # Protocol Data (csv), Protocol Deviation (csv),
    ...
    if zipfile.is_zipfile(unknown_file):
        ...
        if np.all([x in table for x in ["sample_name", "project_code",
                                        "replicate"]]):
            return 'replicate_table'
        elif np.all([x in table for x in ["protocol_name", "replicate-id",
                                          "parameter_name", "parameter_value"]]):
...
def parse_csv_or_tsv(table_file):
    ...
    ttable=None
    try:
        ctable = pd.read_table(io.StringIO(table_string), sep=",",index_col=None, header=1 )
    except:
        pass
    try:
